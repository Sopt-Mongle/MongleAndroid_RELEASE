...
// 프로그래스바 애니메이션 확장함수
fun android.os.Handler.forProgressBar(ProgressBar: ProgressBar,dur: Int){
    android.os.Handler().postDelayed({
        val progressAnimator = ObjectAnimator.ofInt(ProgressBar, "progress", dur, dur+50)
...
fun TextView.forProgressOn(outView: View, inView: View){
    if(this.text.isNotEmpty()){
        outView.setBackgroundResource(R.drawable.dot_circle_progresson_out)
        inView.setBackgroundResource(R.drawable.dot_circle_progresson_in)
    }else{
