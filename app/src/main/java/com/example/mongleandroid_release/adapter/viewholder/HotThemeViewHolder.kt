...
class HotThemeViewHolder(itemView: View) :  RecyclerView.ViewHolder(itemView){
    ...
    fun bind(mainThemes: ResponseMainHotThemeData){

        theme.text = mainThemes.toString()
        sentenceNum.text = sentenceNum.toString()
        Glide.with(itemView).load(themeImg).into(themeImg)
    }
}