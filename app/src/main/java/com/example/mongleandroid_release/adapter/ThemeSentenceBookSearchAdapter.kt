...
import com.example.mongleandroid_release.adapter.viewholder.ThemeSentenceBookSearchViewHolder
...
class ThemeSentenceBookSearchAdapter(private val context: Context) : RecyclerView.Adapter<ThemeSentenceBookSearchViewHolder>(){
        var datas = mutableListOf<BookData>()

        override fun onCreateViewHolder(
            parent: ViewGroup,
            viewType: Int
        ): ThemeSentenceBookSearchViewHolder {
            val view = LayoutInflater.from(context).inflate(R.layout.item_writing_sentence_book_result, parent, false)
            return ThemeSentenceBookSearchViewHolder(view)
        }
        ...
        override fun onBindViewHolder(holder: ThemeSentenceBookSearchViewHolder, position: Int) {
