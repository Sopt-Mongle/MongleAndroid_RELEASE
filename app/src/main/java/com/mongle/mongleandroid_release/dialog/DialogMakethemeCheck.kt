...
class DialogMakethemeCheck(context: Context) {
    ...
    fun start() {
        ...
        Glide.with(dlg.context).load(act.writingThemeImg)
            .into(dlg.findViewById<ImageView>(R.id.dialog_writing_theme_check_img_title))
        dlg.findViewById<TextView>(R.id.dialog_writing_theme_check_tv_title).text =
            act.writingThemeData.theme
    ...
    fun setOnOKClickedListener(listener: (String) -> Unit) {
        ...
        maketheme_popup_yes.setOnClickListener { it ->
            Log.d(
                "requestWritingThemeData",
                "${requestWritingThemeData.themeImgIdx} && ${requestWritingThemeData.theme}"
            )

            themePost(requestWritingThemeData)
            act.setContentView(R.layout.writing_theme_finish)

            val name = act.applicationContext.let {
                SharedPreferenceController.getName(context = it)
            }
            act.findViewById<TextView>(R.id.writing_theme_finish_tv).text =
                "${name}" + "님의 테마를 의미있는 문장으로 \n" + "채워보는 건 어떨까요?"
            ...
            act.writing_theme_finish_btn_write.setOnClickListener {
                ...
                intent.putExtra("param2",themeIdx )
                it.context.startActivity(intent)
                act.finish()
            }
        }
    ...
    private fun themePost(body: RequestWritingThemeData) {
        val call: Call<ResponseWritingThemeData> =
            RequestToServer.service.RequestWritingTheme(token = this.let {
                SharedPreferenceController.getAccessToken(context = this.act)
            }, body = body)
            ...
            override fun onResponse(
                ...
                if (response.isSuccessful) {
                    response.body().let { body ->
                        themeIdx = body!!.data
                        Log.e(
                            "ResponseWritingThemeData 통신응답바디",
