...
fun searchEnter(editText: EditText, button: View, cntText: TextView, result: Unit, what :Int){
    editText.setOnKeyListener(View.OnKeyListener { view, keyCode, _ ->

        if (keyCode == KeyEvent.KEYCODE_ENTER) {
            var keyword = editText.text.toString()
            controlEditText(editText, false)
//            controlButton(editText, button, cntText)
            editText.unshowKeyboard()

            if(keyword.isNullOrBlank()){

            }else{
                when(what){
                    0 -> MainActivity.book_result = keyword.trim()
                    1 -> MainActivity.search_result = keyword.trim()
                }
